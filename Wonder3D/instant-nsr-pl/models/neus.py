...
class ContractionType:
    AABB = 'aabb'
    UN_BOUNDED_SPHERE = 'unbounded_sphere'
...
class NeuSModel(BaseModel):
    ...
    def forward_bg_(self, rays):
        ...
        opacity = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=None, n_rays=n_rays)
        depth = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=midpoints, n_rays=n_rays)
        comp_rgb = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=rgb, n_rays=n_rays)
    ...
    def forward_(self, rays):
        ...
        weights_, _ = render_weight_from_alpha(alpha[..., 0], ray_indices=ray_indices, n_rays=n_rays)
        weights = weights_[..., None]
        opacity = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=None, n_rays=n_rays)
        depth = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=midpoints, n_rays=n_rays)
        comp_rgb = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=rgb, n_rays=n_rays)

        comp_normal = accumulate_along_rays(weights[...,0], ray_indices=ray_indices, values=normal, n_rays=n_rays)
