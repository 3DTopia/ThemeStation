...
class RandomCameraDataModuleConfig:
    ...
    input_image_elevation_offset: float = (
        0  # since Wonder3D results have an elevation offset
    )
    light_sample_strategy: str = "dreamfusion"
    batch_uniform_azimuth: bool = True
    render_img: bool = False
    test_fixed_fovy_dist: bool = True
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # input_image_offset
        elevation_deg += torch.sin(azimuth) * self.cfg.input_image_elevation_offset
        elevation = elevation_deg * math.pi / 180
        ...
        up: Float[Tensor, "B 3"] = torch.as_tensor(
            [
                0,
                -math.sin(self.cfg.input_image_elevation_offset * math.pi / 180),
                math.cos(self.cfg.input_image_elevation_offset * math.pi / 180),
            ],
            dtype=torch.float32,
        ...
        # prepare view dict for lpips loss
        # if len(self.view_ray_dict.keys())==0:
        # self.view_ray_dict['front']=self.initialize_view_ray_dict(elevation_deg=5, azimuth_deg=270, camera_distances=1.6, fovy_deg=75)
        ...
        return {
            ...
            "height": self.height,
            "width": self.width,
            "view_ray_dict": self.view_ray_dict,
        }

    def initialize_view_ray_dict(
        self, elevation_deg, azimuth_deg, camera_distances, fovy_deg
    ) -> Dict[str, Any]:
        elevation_deg *= torch.ones(self.batch_size)
        azimuth_deg *= torch.ones(self.batch_size)
        camera_distances *= torch.ones(self.batch_size)
        fovy_deg *= torch.ones(self.batch_size)
        ...
        # sample light distance from a uniform distribution bounded by light_distance_range
        light_distances: Float[Tensor, "B"] = torch.ones(self.batch_size) * 1.15
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if self.cfg.render_img:
            ...
            if self.cfg.test_fixed_fovy_dist:
                camera_distances = torch.full_like(
                    elevation_deg, self.cfg.eval_camera_distance
                )
                fovy_deg = torch.full_like(elevation_deg, self.cfg.eval_fovy_deg)
            else:
                camera_distances = (
                    torch.rand(self.n_views)
                    * (self.camera_distance_range[1] - self.camera_distance_range[0])
                ...
                fovy_deg = (
                    torch.rand(self.n_views) * (self.fovy_range[1] - self.fovy_range[0])
                    + self.fovy_range[0]
                )
        else:
            if self.split == "val" or self.cfg.render_img:
                # make sure the first and last view are not the same
                azimuth_deg = torch.linspace(0, 360.0, self.n_views + 1)[: self.n_views]
            else:
                azimuth_deg = torch.linspace(0, 360.0, self.n_views)

            elevation_deg = torch.full_like(azimuth_deg, self.cfg.eval_elevation_deg)
            camera_distances = torch.full_like(
                elevation_deg, self.cfg.eval_camera_distance
            )
            fovy_deg = torch.full_like(elevation_deg, self.cfg.eval_fovy_deg)

        # make the front view as the first frame
        azimuth_deg -= 90.0

        # input image_elevation_offset
        azimuth = azimuth_deg * math.pi / 180
        elevation_deg += torch.sin(azimuth) * self.cfg.input_image_elevation_offset
        elevation = elevation_deg * math.pi / 180
        ...
        camera_embeddings: Float[Tensor, "Nv 4"] = torch.cat(
            [elevation[:, None], azimuth[:, None]], dim=-1
        )  # (Nv, 4)
    ...
    def __getitem__(self, index):
        return {
            ...
            "camera_embeddings": self.camera_embeddings[index],
        }
