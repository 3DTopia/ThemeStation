...
from threestudio.utils.misc import scale_anything
from threestudio.utils.misc import update_module_step
...
class TetrahedraSDFGrid(BaseExplicitGeometry):
    ...
    def forward(
        self, points: Float[Tensor, "*N Di"], output_normal: bool = True
        ...
        out = (
            self.feature_network(enc)
            .view(*points.shape[:-1], self.cfg.n_feature_dims)
            .float()
        )
        sdf, features = out[..., 0], out
        output = {"sdf": sdf}
        ...
        if output_normal:
            if self.cfg.normal_type == "finite_difference":
                ...
                points_d_ = (points_unscaled[..., None, :] + offsets).clamp(
                    -self.cfg.radius, self.cfg.radius
                )
                points_d = scale_anything(
                    points_d_, (-self.cfg.radius, self.cfg.radius), (0, 1)
                )
                points_d_sdf = (
                    self.feature_network(self.encoding(points_d.view(-1, 3)))[..., 0]
    ...
    def create_from(
        ...
        **kwargs,
        ...
        if isinstance(other, TetrahedraSDFGrid):
            ...
            if len(instance.sdf.data.shape) == 1:  # sdf should be [Nv, 1]
                instance.sdf.data = instance.sdf.data[:, None]
            ...
            if not instance.cfg.geometry_only and copy_net:
                instance.encoding.load_state_dict(other.encoding.state_dict())
                instance.feature_network.load_state_dict(
                    other.network.state_dict()  # network in Wonder3D = feature_network in 3studio
                )
    ...
    def export(self, points: Float[Tensor, "*N Di"], **kwargs) -> Dict[str, Any]:
        ...
        out = self(points, **kwargs)
        return out

    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        global_step += self.cfg.pos_encoding_config.global_step_offset
        if (
