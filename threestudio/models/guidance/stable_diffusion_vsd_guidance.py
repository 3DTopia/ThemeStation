...
class StableDiffusionVSDGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        pipe=self.get_pipe(self.cfg.pretrained_model_name_or_path, **pipe_kwargs)
    ...
    def get_pipe(self, pretrained_model_name_or_path, **pipe_kwargs) -> StableDiffusionPipeline:
        threestudio.info(f'Load stable diffusion pipeline: {pretrained_model_name_or_path}')

        if self.cfg.dreambooth_ckpt_path:
            ckpt_path=self.cfg.dreambooth_ckpt_path

            threestudio.info(f'Load pretrained dreambooth ckpt from {ckpt_path}')

            threestudio.info(f'Load pretrained dreambooth ckpt.unet.........')
    ...
    def compute_grad_vsd(
        ...
        with torch.no_grad():
            ...
            if self.cfg.use_camera_embedding:
                cross_attention_kwargs = {"scale": 0.0} if self.single_model else None
    ...
    def forward(
        ...
        **kwargs,
        ...
        div=batch_size
        loss_vsd = 0.5 * F.mse_loss(latents, target, reduction="sum") / div
