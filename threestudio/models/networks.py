...
class ProgressiveBandHashGrid(nn.Module, Updateable):
    ...
    def update_step(self, epoch, global_step, on_load_weights=False):
        if "global_step_offset" in self.config.keys():
            global_step+=self.config["global_step_offset"]
...
# class in Wonder3D
class VanillaMLPWon3D(nn.Module):
...
def get_mlp(n_input_dims, n_output_dims, config) -> nn.Module:
    ...
    elif config.otype == "VanillaMLPWon3D":
        network = VanillaMLPWon3D(n_input_dims, n_output_dims, config_to_primitive(config))
