...
class PromptProcessorOutput:
    ...
    def get_view_dependent_prompt(
        ...
        # Get direction
        # azi=(azimuth + 90 + 180) % 360 - 180
        prompts_v=[]
        for d in self.directions:
            if d.condition(elevation, azimuth, camera_distances):
                prompts_v.append(d.prompt(prompt))
        if len(prompts_v)>1:
            print(prompts_v)
...
def shift_azimuth_deg(azimuth: Float[Tensor, "..."]) -> Float[Tensor, "..."]:
    ...
    shift_wonder_3d=True # TODO: make this as config
    azi=(azimuth + 180) % 360 - 180
    if shift_wonder_3d:
        if azi[0]>90 and azi[0]<=180:
            azi-=270
        else:
            azi+=90
    return azi
...
class PromptProcessor(BaseObject):
    ...
    @staticmethod
    def spawn_func(pretrained_model_name_or_path, prompts, cache_dir, dreambooth_ckpt_path):
        raise NotImplementedError
    ...
    def prepare_text_embeddings(self):
        ...
        threestudio.info(
                        f'prompts_to_process:{prompts_to_process}'
                    )
