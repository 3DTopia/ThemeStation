...
class StableDiffusionPromptProcessor(PromptProcessor):
    ...
    def configure_text_encoder(self) -> None:
        ...
        if self.cfg.dreambooth_ckpt_path:
            self.text_encoder = CLIPTextModel.from_pretrained(
                f'{self.cfg.dreambooth_ckpt_path}/text_encoder'
                ).to(self.device)
        else:
            self.text_encoder = CLIPTextModel.from_pretrained(
    ...
    @staticmethod
    def spawn_func(pretrained_model_name_or_path, prompts, cache_dir, dreambooth_ckpt_path=None):
        os.environ["TOKENIZERS_PARALLELISM"] = "false"
        tokenizer = AutoTokenizer.from_pretrained(
            pretrained_model_name_or_path, subfolder="tokenizer"
        )

        if dreambooth_ckpt_path:
            threestudio.info(f'SD prompt processor: {pretrained_model_name_or_path}')
            threestudio.info(f'Load pretrained dreambooth text encoder from {dreambooth_ckpt_path}')
            text_encoder = CLIPTextModel.from_pretrained(
                f'{dreambooth_ckpt_path}/text_encoder',
                device_map="auto",
                )
        else:
            text_encoder = CLIPTextModel.from_pretrained(
