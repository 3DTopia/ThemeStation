    ...
    def forward(
        ...
        **kwargs
        ...
        rast, _ = self.ctx.rasterize(
            v_pos_clip, mesh.t_pos_idx, (height, width)
        )  # v_pos_clip.shape=[1,0,4]
        mask = rast[..., 3:] > 0
        ...
        if render_rgb:
            ...
            geo_out = self.geometry(
                positions, output_normal=True
            )  # default: output_normal=False
            extra_geo_info = {}
            ...
            if "use_white_background" in kwargs.keys():
                if kwargs["use_white_background"]:
                    gb_rgb_bg = torch.ones_like(gb_rgb_bg).to(gb_rgb_bg)
