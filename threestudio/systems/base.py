...
class BaseLift3DSystem(BaseSystem):
    ...
    class Config(BaseSystem.Config):
        ...
        prev_cfg_dir: Optional[str] = None
        ...
        ref_guidance: dict = field(default_factory=dict)

        ref_norm_guidance: dict = field(default_factory=dict)

        prompt_processor_type: str = ""
        prompt_processor: dict = field(default_factory=dict)
        prompt_norm_processor: dict = field(default_factory=dict)

        ref_prompt_processor: dict = field(default_factory=dict)
        ref_prompt_norm_processor: dict = field(default_factory=dict)
    ...
    def configure(self) -> None:
        if (
            ...
            prev_cfg = load_config(
                os.path.join(
                    os.path.dirname(self.cfg.prev_cfg_dir),
                    "../configs/parsed.yaml",
                )
            ...
            state_dict, epoch, global_step = load_module_weights(
                ...
                module_name="model.geometry",  # HARD_CODE load weights from Wonder3D ckpt
                map_location="cpu",
            )

            # not model.geometry
            if not len(state_dict.keys()) > 0:
                state_dict, epoch, global_step = load_module_weights(
                    self.cfg.geometry_convert_from,
                    module_name="geometry",  # HARD_CODE load weights from threestudio ckpt
                    map_location="cpu",
                )

            # state_dict['sdf']=state_dict['sdf'][:, None]
        ...
        if (
            ...
            if self.cfg.material_convert_from == "same_as_geometry":
                self.cfg.material_convert_from = self.cfg.geometry_convert_from
            state_dict, epoch, global_step = load_module_weights(
                self.cfg.material_convert_from,
                module_name="model.texture",  # HARD_CODE material named texture in Wonder3D
                map_location="cpu",
            )
            ...
            if not len(state_dict.keys()) > 0:
                state_dict, epoch, global_step = load_module_weights(
                    self.cfg.geometry_convert_from,
                    module_name="material",  # HARD_CODE load weights from threestudio ckpt
                    map_location="cpu",
                )
            ...
            # convert from coarse stage material
            prev_material = prev_material.to(get_device())
            self.material = threestudio.find(self.cfg.material_type).create_from(
                prev_material, self.cfg.material
            )
            del prev_material
            cleanup()
        else:
